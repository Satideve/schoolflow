    ...
    <div className="space-y-6">
      ...
      <section className="bg-white rounded shadow p-4 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block text-sm mb-1">Concession (optional)</label>
              <input
                type="number"
                step="0.01"
                className="w-full border rounded p-2"
          ...
          >
            {createAssignment.isPending ? "Saving..." : "Assign Plan"}
          </button>
        </form>
      </section>
                ...
                {list.map((a: any) => (
                  <tr key={a.id} className="border-b last:border-b-0">
                    ...
                    <td className="py-1 pr-2">
                      {studentNameById.get(a.student_id) ?? `Student #${a.student_id}`}
                    </td>
                    <td className="py-1 pr-2">
                      {feePlanNameById.get(a.fee_plan_id) ?? `Plan #${a.fee_plan_id}`}
                    </td>
                    <td className="py-1 pr-2">{a.concession}</td>
                    <td className="py-1 pr-2">{a.note ?? ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
